...
- [x] be in a version controlled directory linked to a GitHub repo
- [x] use Vite's React template using JavaScript (no TS or SWC)
- [x] start with no console errors or warnings
- [x] render a title and an unordered list of todos

### Instructions Part 1: Repo Setup for the Todo App

#### Create new public repo on GitHub

- [x] give it the name "todo-list" or something similar and description
- [x] don't add a .gitignore or a license
- [x] clone the repo to your local environment (this differs based on if you're using HTTPS, SSH, or GitHub Desktop. See [GitHub's documentation on remote repos](https://docs.github.com/en/get-started/getting-started-with-git/about-remote-repositories) for more details.)
- [x] don't create a new branch yet - remain on `main` branch

### Instructions Part 2: Installation

#### Scaffold Vite Using CLI

- [x] Bootstrap a new project with command: `npm create vite@latest . -- --template react`
- [x] Install with command: `npm install`
...
- [x] Stage all the files with git: `git add .`
- [x] Commit them to main: `git commit -m "installed react"`
- [x] Push the changes to GitHub: `git push`
- [x] Create and check out a new branch for week 1's assignment before continuing: `git checkout -b week-01-setup`
- [x] Publish the branch to github: `git push origin week-01-setup`

#### Clean up Template

- [x] Start the development server with the command: `npm run dev`
- [x] Open a browser to the **Local** link listed in the CLI
- [x] Delete the contents of App.css but keep the file.
- [x] Delete the contents of index.css but keep the file.
- [x] Clean up App's code:
  - [x] Remove all imports except for App.css
  - [x] Delete the line containing `const [count, setCount] = useState(0)`
  - [x] Remove everything from the return statement.
  - [x] In the now empty return statement, add a div containing an h1 for the title for the app
...
- [x] After App's first line but before the return statement, create an array named `todos`containing 3 empty objects. Each one of those objects will represent a todo.
- [x] Populate each object with a todo using the following object keys: `id` and `title`.
...
- [x] Below the h1 in the return statement, create an unordered list using html tags.
- [x] Place an empty code block between the list's opening and closing tags `<ul>{}</ul>`
- [x] Inside the code block, map the todos to html that will render a list item per todo.
...
- [x] in VS Code, click on the extensions icon in the left-hand sidebar
...
- [x] search for "ESLint". The appropriate extension will have the following icon:
...
- You need to add another plugin package, eslint-plugin-react, for additional usefulReact linting rules:
  - [x] In the terminal, install it using `npm install eslint-plugin-react --save-dev`
  - [x] Go to `eslint.config.js` add the import statement `import react from 'eslint-plugin-react';` above the `reactHooks` import.
  - [x] In the second object in the exported array:
    - [x] Add a `settings` object: `settings: { react: { version: 'detect' } },`.
    - This should be placed just above the `plugins` object.
  - [x] Add `react` to the `plugins` object, above `react-hooks: reactHooks`. _tip: You don't need to include a key/value pair like the other plugins._
  - [x] Add the following to the `rules` below `...js.configs.recommended.rules,`
    - [x] `...react.configs.recommended.rules,`
    - [x] `...react.configs['jsx-runtime'].rules,`
- [x] Take some time to review the `eslint.config.js` file you just updated. Documentation for the [ESLint plugin](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint) and [ESLint library](https://eslint.org/) can explain most of the contents.
- [x] Finally, we recommend adding the following entries to the `rules` object:
...
- [x] search for "Prettier". Choose the following from the results:
...
- [x] after installed, go to VS Code's settings and search for "format"
  - [x] find the "Default Formatter" setting and change the option to Prettier - Code formatter.
  - [x] If you've previously modified VS Code settings, refer to the full installation instructions on the [Prettier's VS Code Extension homepage](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode) to to help with troubleshooting.

##### Install Prettier in the Project

- [x] In the terminal issue the command: `npm install --save-dev --save-exact prettier`
- [x] Create a .prettierrc file at the root of the project directory
...
- [x] Open the README.md in the root of the project and empty its contents.
- [x] Write some basics about the project. Include at least the following:
  - [x] App name and description
  - [x] Installation instructions
  - [x] How to run the development server

#### Version Control Closeout Tasks

- [x] Commit changes to your local working branch.
- [x] Push the changes up to GitHub (publish the branch there if you have not done so already).
